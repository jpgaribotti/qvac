...
- **Desktop** (Linux, macOS, Windows): consumer addons dynamically link against
  ...
  process. On **Linux**, ggml compute backends are separate `.so` files under
  `prebuilds/<platform>/qvac__fabric/` and load via
  `ggml_backend_load_all_from_path()`. On **macOS and Windows**, the backends are
  static inside `qvac__fabric.bare` and self-register on load.
