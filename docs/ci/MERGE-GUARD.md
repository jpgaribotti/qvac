...
When you add a C++-test-bearing addon, add its key to `CPP_TEST_KEYS` and to the `changes` paths-filter in `pr-gate-merge.yml`, and make sure its producer posts `qvac/cpp-tests-<key>` (`vla` for `packages/vla-ggml`).
