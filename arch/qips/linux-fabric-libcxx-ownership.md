...
- `-nostdlib++` suppresses the driver's stdlib link outright rather than changing how it links, so the directory-scoped `-static-libstdc++` it sits alongside becomes inert; the two need no ordering and `project_setup` is untouched.
